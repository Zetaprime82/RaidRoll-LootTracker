--
--
local function _sliderOnValueChangedFix(slider, value)
    if not slider._onsetting then  -- is single threaded
        ...
        return value -- ignore recursion for actual event handler
    end
end
...
function Setup_RR_Panel()
    ...
    RaidRoll_PriorityPanel = CreateFrame("Frame", "Raid Roll Priority Panel", UIParent)
    -- Register in the Interface Addon Options GUI
    -- Set the name for the Category for the Options Panel
    ...
    local version = GetAddOnMetadata("RaidRoll", "Version")
    if version then
        version = " - " .. version
    else
        version = ""
    end
    Raid_Roll_PanelName_String:SetText("|cFFC41F3B" .. RAIDROLL_LOCALE["OPTIONSTITLE"] .. version)
    ...
    -- [ ] Silence Loot Reason
    RR_RollCheckBox_SilenceLootReason = CreateFrame("CheckButton", "RR_RollCheckBox_SilenceLootReason", RaidRoll_Panel.panel, "UICheckButtonTemplate")
    RR_RollCheckBox_SilenceLootReason:SetWidth(20)
    RR_RollCheckBox_SilenceLootReason:SetHeight(20)
    RR_RollCheckBox_SilenceLootReason:SetPoint("TopLeft", RR_RollCheckBox_Multi_Rollers, "BottomLeft", 0, 0)
    _G["RR_RollCheckBox_SilenceLootReason" .. "Text"]:SetText("|cFFFFFFFF" .. RAIDROLL_LOCALE["SilenceLootReason"])
    if (GetLocale() ~= "zhTW" and GetLocale() ~= "ruRU") then
        _G["RR_RollCheckBox_SilenceLootReason" .. "Text"]:SetFont("Fonts\\FRIZQT__.TTF", 12)
    end
    if (GetLocale() == "zhCN") then
        _G["RR_RollCheckBox_SilenceLootReason" .. "Text"]:SetFont("Fonts\\ZYKai_T.TTF", 15)
    end
    RR_RollCheckBox_SilenceLootReason:SetScript("OnClick", function(self) RaidRoll_DBPC[UnitName("player")][self:GetName()] = self:GetChecked() end)
    RR_RollCheckBox_SilenceLootReason:SetScript("OnEnter", function(self) RR_MouseOverTooltip(self:GetName()) end)
    RR_RollCheckBox_SilenceLootReason:SetScript("OnLeave", function() GameTooltip:Hide() end)
    RR_RollCheckBox_SilenceLootReason:SetChecked(RaidRoll_DBPC[UnitName("player")]["RR_RollCheckBox_SilenceLootReason"])
    ...
    -- [ ] Also show roll value
    RAIDROLL_LOCALE["EPGP_Also_Show_Roll"] = "Also show roll value"
    RAIDROLL_LOCALE["RR_RollCheckBox_EPGP_Also_Show_Roll"] = "In addition to the PR value,\nalso show the roll value.\n\nRecommended: Off"
    RR_RollCheckBox_EPGP_Also_Show_Roll = CreateFrame("CheckButton", "RR_RollCheckBox_EPGP_Also_Show_Roll", RaidRoll_Panel.panel, "UICheckButtonTemplate")
    RR_RollCheckBox_EPGP_Also_Show_Roll:SetWidth(15)
    RR_RollCheckBox_EPGP_Also_Show_Roll:SetHeight(15)
    RR_RollCheckBox_EPGP_Also_Show_Roll:SetPoint("TopLeft", RaidRoll_Panel.panel, "TopLeft", 40, -295)
    _G["RR_RollCheckBox_EPGP_Also_Show_Roll" .. "Text"]:SetText("|cFFFFFFFF" .. RAIDROLL_LOCALE["EPGP_Also_Show_Roll"])
    if (GetLocale() ~= "zhTW" and GetLocale() ~= "ruRU") then
        _G["RR_RollCheckBox_EPGP_Also_Show_Roll" .. "Text"]:SetFont("Fonts\\FRIZQT__.TTF", 10)
    end
    if (GetLocale() == "zhCN") then
        _G["RR_RollCheckBox_EPGP_Also_Show_Roll" .. "Text"]:SetFont("Fonts\\ZYKai_T.TTF", 14)
    end
    RR_RollCheckBox_EPGP_Also_Show_Roll:SetScript("OnClick", RaidRoll_CheckButton_Update_Panel)
    RR_RollCheckBox_EPGP_Also_Show_Roll:SetScript("OnEnter", function(self) RR_MouseOverTooltip(self:GetName()) end)
    RR_RollCheckBox_EPGP_Also_Show_Roll:SetScript("OnLeave", function() GameTooltip:Hide() end)
    ...
    Raid_Roll_ScalePanelName_String:SetPoint("TopLeft", RaidRoll_Panel.panel, "TOPLEFT", 20, -320)
    ...
    RaidRoll_Scale_Slider:SetPoint('TOPLEFT', 20, -345)
    RaidRoll_Scale_Slider:SetOrientation('HORIZONTAL')

    if RaidRoll_DB["Scale"] == nil then
        RaidRoll_DB["Scale"] = 100
    else
        ...
        -- In case these slider value is fractional, make it integral.
        RaidRoll_DB["Scale"] = floor(RaidRoll_DB["Scale"])
    end
    ...
    RaidRoll_Scale_Slider:SetScript("OnValueChanged", function(self, value)
        value = _sliderOnValueChangedFix(self, value)
        RR_RollFrame:SetScale(value / 100)
        _G[self:GetName() .. 'Text']:SetText(value .. "%")
        RaidRoll_DB["Scale"] = value
    end)
    ...
    Raid_Roll_PanelName_String = RaidRoll_Panel.panel:CreateFontString("Raid_Roll_PanelName_String", "ARTWORK", "GameFontNormal")
    Raid_Roll_PanelName_String:SetPoint("TopLeft", RaidRoll_Panel.panel, "TOPLEFT", 20, -375)
    Raid_Roll_PanelName_String:SetJustifyH("LEFT")
    ...
    RaidRoll_ExtraWidth_Slider:SetPoint('TOPLEFT', 20, -400)
    ...
    if RaidRoll_DBPC[UnitName("player")]["RR_ExtraWidth"] == nil then
        ...
        -- will receive fractional values; as detailed at http://www.wowwiki.com/Patch_5.4.0/API_changes#Slider.
        -- In case these slider value is fractional, make it integral.
        RaidRoll_DBPC[UnitName("player")]["RR_ExtraWidth"] = floor(RaidRoll_DBPC[UnitName("player")]["RR_ExtraWidth"])
    end
    ...
    RaidRoll_ExtraWidth_Slider:SetScript("OnValueChanged", function(self, value)
        value = _sliderOnValueChangedFix(self, value)
        RaidRoll_CheckButton_Update_Panel()
        _G[self:GetName() .. 'Text']:SetText(value)
        RaidRoll_DBPC[UnitName("player")]["RR_ExtraWidth"] = value
    end)
    ...
    -- Set Extra Rank Width
    Raid_Roll_PanelName_String = RaidRoll_Panel.panel:CreateFontString("Raid_Roll_PanelName_String", "ARTWORK", "GameFontNormal")
    Raid_Roll_PanelName_String:SetPoint("TopLeft", RaidRoll_Panel.panel, "TOPLEFT", 240, -375)
    Raid_Roll_PanelName_String:SetJustifyH("LEFT")
    ...
    RaidRoll_Rolling_Time_Slider:SetPoint('TOPLEFT', 240, -400)
    ...
    if RaidRoll_DBPC[UnitName("player")]["Time_Offset"] == nil then
        ...
        -- will receive fractional values; as detailed at http://www.wowwiki.com/Patch_5.4.0/API_changes#Slider.
        -- In case these slider value is fractional, make it integral.
        RaidRoll_DBPC[UnitName("player")]["Time_Offset"] = floor(RaidRoll_DBPC[UnitName("player")]["Time_Offset"])
    end
    ...
    RaidRoll_Rolling_Time_Slider:SetScript("OnValueChanged", function(self, value)
        value = _sliderOnValueChangedFix(self, value)
        RaidRoll_CheckButton_Update_Panel()
        _G[self:GetName() .. 'Text']:SetText(value + 60)
        RaidRoll_DBPC[UnitName("player")]["Time_Offset"] = value
    end)
...
function RR_Priority_Modify(Priority_Index)
    RR_Debug("Priority_Index" .. Priority_Index)
...
function RaidRoll_CheckButton_Update_Panel()
    ...
    if RR_RollCheckBox_EPGP_Also_Show_Roll:GetChecked() then
    ...
    -- Update size of roll tracking frame based on new options
